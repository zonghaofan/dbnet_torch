...
def init_args():
    ...
    parser.add_argument('--config_file', default='/red_detection/DBNet/DBNet_fzh/config/icdar2015_resnet18_FPN_DBhead_polyLR_code_phone.yaml', type=str)
...
def train(model, optimizer, epochs, criterion, train_loader, config, post_process, validate_loader,output_path):
    for epoch_index in range(epochs):
        ...
        for batch_index, batch in enumerate(train_loader):
            ...
            lr = ajust_learning_tri(optimizer, batch_index_, step_size=len(train_loader) * 10,
                                    base_lr=float(config['optimizer']['args']['base_lr']),
                                    max_lr=float(config['optimizer']['args']['max_lr']))
            # 数据进行转换和丢到gpu
            for key, value in batch.items():
                if value is not None:
                    if isinstance(value, torch.Tensor):
...
def main_entrance():
    ...
    if config['optimizer']['type'] == 'Adam':
        optimizer = optim.Adam(model.parameters(), lr=float(config['optimizer']['args']['max_lr']),
